...
// Set the view engine to use EJS as well as set default views directory
app.set('view engine', 'ejs');
app.set('views', __dirname + '/public/views/');

// This tell express out of which directory to serve static assets like CSS an images
...
var NON_INTERACTIVE_CLIENT_ID = 'q4FuT4VZsGXEtnDv0TKGQC50LCcl3AIb';
var NON_INTERACTIVE_CLIENT_SECRET = '3JjgtLOd4xk1HYwG8Xk2rGM0AL3fpRLa7_JQbvWnm7oCHULPPNOO-i8MqNLoUcH5';
...
function getAccessToken(req, res, next) {
	request
		.post('https://sbcharjee.auth0.com/oauth/token')
		.send(authData)
...
// The homepage route of our application does not interface with the 
// MovieAnalyst API and is always accessible. We won’t use the getAccessToken middleware here.
 // We’ll simply render the index.ejs view.
app.get('/', function(req, res){
  res.render('index');
})

// For the movies route, we’ll call the getAccessToken middleware to
 // ensure we have an access token. If we do have a valid access_token, 
 // we’ll make a request with the superagent library and we’ll be 
 // sure to add our access_token in an Authorization header before making the request to our API.
// Once the request is sent out, our API will validate that the access_token has the right scope to request 
// the /movies resource and if it does, will return the movie data. We’ll take this movie data, 
// and pass it alongside our movies.ejs template for rendering
...
app.get('/authors', getAccessToken, function(req, res){
  request
    .get('http://localhost:8080/reviewer')
    .set('Authorization', 'Bearer ' + req.access_token)
    .end(function(err, data) {
      if(data.status == 403){
        res.send(403, '403 Forbidden');
...
// MovieAnalyst Website will always result in a 403 Forbidden error as this 
